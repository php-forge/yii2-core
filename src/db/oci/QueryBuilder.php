...
class QueryBuilder extends \yii\db\QueryBuilder
{
     ...
     * {@inheritdoc}
     */
    public function resetAutoIncrement(string $table, string $column, int $value = 1): string
    {
        return <<<SQL
            ALTER TABLE {$this->db->quoteTableName($table)}
            MODIFY {$this->db->quoteColumnName($column)} GENERATED BY DEFAULT ON NULL AS IDENTITY (START WITH $value)
        SQL;
    }
