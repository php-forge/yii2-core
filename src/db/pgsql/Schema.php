...
SQL;
    ...
    {
        $tableName = $this->db->quoteValue($tableSchema->name);
        $schemaName = $this->db->quoteValue($tableSchema->schemaName);
        ...
        SELECT
            ...
            (
                SELECT nspname
                FROM pg_namespace
                WHERE oid = COALESCE(td.typnamespace, tb.typnamespace, t.typnamespace)
            ) AS type_scheme,
        ...
        FROM
            ...
            LEFT JOIN pg_type tb ON (
                (a.attndims > 0 OR t.typcategory = 'A') AND t.typelem > 0 AND t.typelem = tb.oid
    ...
    protected function loadColumnSchema(array $info): ColumnSchema
    {
        ...
        if (
            $info['type_scheme'] !== null &&
            !in_array($info['type_scheme'], [$this->defaultSchema, 'pg_catalog'], true)
        ) {
            $column->dbType = $info['type_scheme'] . '.' . $info['data_type'];
        }

        if (empty($info['sequence_name']) === false) {
