...
{
    /**
     * @event \yii\base\Event an event triggered after a DB connection is established
     */
    public const EVENT_AFTER_OPEN = 'afterOpen';
    /**
     * @event \yii\base\Event an event triggered right before a top-level transaction is started
     */
    public const EVENT_BEGIN_TRANSACTION = 'beginTransaction';
    /**
     * @event \yii\base\Event an event triggered right after a top-level transaction is committed
     */
    public const EVENT_COMMIT_TRANSACTION = 'commitTransaction';
    /**
     * @event \yii\base\Event an event triggered right after a top-level transaction is rolled back
     */
     ...
     * Note that to enable truly schema caching, a valid cache component as specified by [[schemaCache]] must be enabled
     * and [[enableSchemaCache]] must be set true.
     ...
     * Note that to enable query caching, a valid cache component as specified by [[queryCache]] must be enabled and
     * [[enableQueryCache]] must be set true.
    ...
    /**
     * @var CacheInterface|string|null the cache object or the ID of the cache application component that is used for
     * query caching.
     * @see enableQueryCache
     */
    public CacheInterface|string|null $queryCache = 'cache';
    /**
     * @var string|null the charset used for database connection. The property is only used for MySQL, PostgresSQL
    ...
    /**
     * @var bool|null whether to turn on prepare emulation. Defaults to false, meaning PDO will use the native preparing
     * support if available. For some databases (such as MySQL), this may need to be set true so that PDO can emulate
     * the prepared support to bypass the buggy native prepare support.
     ...
     * percentage character `%` will be replaced with this property value.
     *
     ...
     * The keys of the array are PDO driver names while the values are either the corresponding schema class names or
     * configurations.
     *
     * Please refer to [[Yii::createObject()]] for details on how to specify a configuration.
     *
     * This property is mainly used by [[getSchema()]] when fetching the database schema information.
     * You normally do not need to set this property unless you want to use your own [[Schema]] class to support DBMS
     * that is not supported by Yii.
     */
    public array $schemaMap = [
        'pgsql' => \yii\db\pgsql\Schema::class, // PostgresSQL
     ...
     * @see noCache()
     */
    public function cache(callable $callable, int|null $duration = null, Dependency|null $dependency = null): mixed
    {
     ...
     * @internal
     */
    public function getQueryCacheInfo(int|null $duration, Dependency|null $dependency): array|null
    {
     ...
     * [[Open]] calls this method to establish a DB connection.
    ...
    protected function createPdoInstance(): PDO
    {
        ...
        if ($pdoClass === null) {
            ...
            $pdoClass = match ($driver) {
                'mssql' => \yii\db\mssql\PDO::class,
                'dblib' => \yii\db\mssql\DBLibPDO::class,
                'sqlsrv' => \yii\db\mssql\SqlsrvPDO::class,
                default => PDO::class,
            };
        }
     ...
     * @throws InvalidConfigException if the isolation level is not supported.
     * @throws NotSupportedException if transactions are not supported by the underlying DBMS or the connection is not
     ...
     * In some cases rollback can fail, so this method is fail-safe.
     * Exception thrown from rollback will be caught and just logged with [[\Yii::error()]].
     *
     ...
     *
     * @throws NotSupportedException|InvalidConfigException if there is no support for the current driver type.
     */
    ...
    /**
     * Collects the schema information for the named table.
     *
    ...
    public function quoteColumnName(string $name): string
    {
        return $this->_quotedColumnNames[$name]
            ?? ($this->_quotedColumnNames[$name] = $this->getQuoter()->quoteColumnName($name));
    }
    ...
    public function quoteTableName(string $name): string
    {
        return $this->_quotedTableNames[$name]
            ?? ($this->_quotedTableNames[$name] = $this->getQuoter()->quoteTableName($name));
    }

    /**
     * Processes an SQL statement by quoting table and column names that are enclosed within double brackets.
     * Tokens enclosed within double curly brackets are treated as table names, while tokens enclosed within double
     * square brackets are column names.
     * They will be quoted accordingly.
   ...
   /**
     ...
     * Based on the current [[dsn]], in case it was not set explicitly by an end user.
     *
    ...
    public function getDriverName(): string|null
    {
        if ($this->_driverName === '') {
            if ($this->dsn !== null && ($pos = strpos($this->dsn, ':')) !== false) {
                ...
                $this->_driverName = strtolower(
                    $this->getSlavePdo()?->getAttribute(PDO::ATTR_DRIVER_NAME)
                );
            }
        }
     ...
     */
    public function getSlavePdo(bool $fallbackToMaster = true): PDO|null
    {
     ...
     * ```
     *
     * @param callable $callback a PHP callable to be executed by this method.
     * Its signature is `function (Connection $db)`.
     ...
     *
     * This method implements load balancing and fail over among the given list of the servers.
     * Connections will be tried in random order.
     * For details about the fail over behavior, see [[openFromPoolSequentially]].
     *
     ...
     * Opens the connection to a server in the pool.
     *
     * This method implements fail over among the given list of servers.
    ...
    public function __sleep(): array
    {
        ...
        unset(
            $fields['pdo'],
            $fields["\000" . __CLASS__ . "\000" . '_master'],
            $fields["\000" . __CLASS__ . "\000" . '_slave'],
            $fields["\000" . __CLASS__ . "\000" . '_transaction'],
            $fields["\000" . __CLASS__ . "\000" . '_schema'],
        );
