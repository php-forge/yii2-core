 ...
 * In the following example the filter will be applied to the `index` action and cache the whole page for maximum 60
 * seconds or until the count of entries in the post table changes.
...
class PageCache extends ActionFilter implements DynamicContentAwareInterface
{
     ...
     * After the PageCache object is created, if you want to change this property, you should only assign it with a
     * cache object.
     */
     ...
     *
     * If [[cacheCookies]] or [[cacheHeaders]] is enabled, then [[\yii\caching\Dependency::reusable]] should be enabled
     * as well to save performance.
     * This is because the cookies and headers are currently stored separately from the actual page content, causing the
     * dependency to be evaluated twice.
     */
     ...
     * ```
     *
     * You can provide an anonymous function to generate variations. This is especially helpful when you need to access
     * the User component, which is resolved before the PageCache behavior:
     *
     * ```php
     * 'variations' => function() {
     *     return [
     ...
     */
    public array|string|Closure $variations = [];
    /**
    ...
    /**
     * @var View|null the view component to use for caching. If not set, the default application view component
     * [[\yii\web\Application::view]] will be used.
     */
    public View|null $view = null;
    /**
     * @var bool|array a boolean value indicating whether to cache all cookies, or an array of cookie names indicating
     * which cookies can be cached. Be very careful with caching cookies, because it may leak sensitive or private data
     * stored in cookies to unwanted users.
     */
    public bool|array $cacheCookies = false;
    /**
     ...
     */
    public bool|array $cacheHeaders = true;
     ...
     */
    public function beforeCacheResponse(): bool|array
    {
        return true;
    }
     ...
     */
    public function afterRestoreResponse(array|null $data): void
    {
    }
    ...
    public function cacheResponse(): void
    {
        ...
        $this->cache->getOrset($this->calculateCacheKey(), static fn() => $data, $this->duration, $this->dependency);
    ...
    /**
     * @return string the key used to cache response properties.
     */
